...
export default function useErrorServerHandler(
  serverErrorMessages,
  registerFromInputs,
) {
  const error = ref({
  ...
  const checkServerErrors = (errorType) => {
    if (
      !isNotEmptyObject(serverErrorMessages) ||
      !isNotEmptyString(errorType)
    ) {
      return;
    }
  ...
  const getErrorMessage = (errorType) => {
    return (
      serverErrorMessages[errorType] || serverErrorMessages["default_error"]
    );
  };
